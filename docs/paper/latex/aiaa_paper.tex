...
AstraHeal integrates five core modules into a closed-loop pipeline: (1) Causal Preprocessing ($\frac{dV}{dt}, \frac{dT}{dt}, \widehat{R}_{\text{int}}$); (2) Ensemble Anomaly Detection ($\text{AUROC} = 0.974$); (3) Dirichlet Evidential Bayesian Diagnosis ($u_{\text{epistemic}}, u_{\text{aleatoric}}$); (4) Zero-Mutation Digital Twin Counterfactual Lookahead (3000s horizon); and (5) Deterministic Safety Governor with Communication-Aware Urgency Arbitration.
